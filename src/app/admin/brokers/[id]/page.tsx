...
export default function AdminBrokerViewPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      const [profileRes, loadsRes] = await Promise.all([
        client.from('profiles').select('email, full_name').eq('id', id).single(),
        client.from('loads').select('id, load_number, status, origin_city, origin_state, dest_city, dest_state, commodity, weight_lbs').eq('broker_profile_id', id).order('created_at', { ascending: false }),
      ])
      if (cancelled) return
      if (profileRes.error) {
    ...
    <div className="min-h-screen bg-background text-[#F9FAFB]">
      ...
      <main className="p-4 sm:p-6 max-w-4xl mx-auto">
        ...
        {loading ? (
          ...
          <p className="text-[#A3A3A3]">Broker not found.</p>
        ) : (
          <>
            <div className="mb-8 flex items-center gap-3">
              <User className="h-8 w-8 text-primary" />
              <div>
                <h1 className="font-display text-xl font-bold text-primary">{profile.full_name || profile.email}</h1>
                <p className="text-sm text-[#A3A3A3]">{profile.email}</p>
                <p className="text-xs text-[#525252] mt-0.5">Broker · Full access view (admin/support)</p>
              </div>
            </div>
